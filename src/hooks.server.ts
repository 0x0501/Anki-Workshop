...
const corsMiddleware: Handle = async ({ event, resolve }) => {
	...
	response.headers.set('Access-Control-Allow-Origin', 'https://your-trusted-domain.com');
	response.headers.set('Access-Control-Allow-Methods', 'GET, POST, PUT, DELETE, OPTIONS');
	response.headers.set('Access-Control-Allow-Headers', 'Content-Type, Authorization');
	return response;
};

// This code will catch all the `throw` clause, and recast it into a formatted json
